...
unsigned int motor1_last=0;   //lasttime 计左电机码盘脉冲值
unsigned int motor2_last=0;   //lasttime 计右电机码盘脉冲值
...
const int leftmotorpin1 = 7; // 直流电机信号 IN1~4
...
void setup() {
  ...
  pinMode(3, INPUT_PULLUP); // left_motor triger rising edge in counting module, and module should connect to pin3
  pinMode(2, INPUT_PULLUP); // similar as above.
...
void loop() {
  // try and test time interval and to calculate speef
  try_speed_cal();
  
  // read in 20 characters from buffer
  for(int i=0; i<20; i++){
    if(BT.available()){
      oneline[i] = BT.read();
    }
  ...
  convert_command[8]=0; // end string
